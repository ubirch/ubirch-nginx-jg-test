  ...
  def addVertexWithProperties(): Unit = {
    ...
    try {
      ...
      case e: CompletionException =>
        ...
        throw new ImportToGremlinException(s"Error on adding properties to vertex ${coreVertex.toString}: " + e.getMessage)
      case e: Exception => throw e
    }
  }

  private def initialiseVertex: Vertex = {
    g.addV(coreVertex.label).property(coreVertex.properties.head.toKeyValue).l().head
  }
